...
const basePath = process.env.BASE_PATH || "/nchu-project";
...
const getOfficialSources = async (question) => {
  ...
  for (const source of sourceCatalog.filter((source) => source.patterns.some((pattern) => pattern.test(question)))) {
    ...
    try {
      ...
      // Public pages change or block requests; keep other sources working.
    }
  }
...
const handleApi = async (req, res, requestUrl) => {
  try {
    ...
    if (requestUrl.pathname === "/api/nchu/chat") {
      ...
      const answer = await createAiAnswer(await getRequestBody(req));
...
const serveStatic = async (req, res, requestUrl) => {
  ...
  const filePath = requested.startsWith(distRoot) && existsSync(requested) ? requested : join(distRoot, "index.html");
...
createServer(async (req, res) => {
  ...
  if (requestUrl.pathname.startsWith("/api/nchu/") && (await handleApi(req, res, requestUrl))) return;
  ...
  console.log(`NCHU Study Assistant listening on http://0.0.0.0:${port}${basePath}/`);
});
